...
from videollava.constants import DEFAULT_IMAGE_TOKEN, DEFAULT_VIDEO_TOKEN, IMAGE_TOKEN_INDEX, DEFAULT_VID_START_TOKEN, DEFAULT_VID_END_TOKEN
...
def parse_args():
    ...
    parser.add_argument('--load_peft', help='', default=None)
...
def get_model_output(model, video_processor, tokenizer, video, qs, args, num_frames=8):
    vid_replace_token = DEFAULT_IMAGE_TOKEN * num_frames
    if model.config.mm_use_im_start_end:
        vid_replace_token = DEFAULT_VID_START_TOKEN + vid_replace_token + DEFAULT_VID_END_TOKEN
        qs = qs.replace(DEFAULT_VIDEO_TOKEN, vid_replace_token)
...
def run_inference(args):
    ...
    if args.load_peft is not None:
        ...
        # model.load_adapter(args.load_peft, adapter_name='self')
    ...
    for sample in tqdm(gt_questions):
        ...
        sample_set = {'id': id, 'type': qtype, 'question': question, 'answer': answer}

        # Load the video file
        temp_path = os.path.join(args.video_dir, f"{video_path}")
        if os.path.exists(temp_path):
            try:
                ...
                print(f"Error processing file {temp_path}, Exception {e}, skipping ...")
        else:
