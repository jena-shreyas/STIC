...
video_path = "pert_vids/noise_uniform/stock-footage-a-cute-dog-lies-on-grass-and-dozes.mp4"
...
inputs = processor(text=prompt, videos=clip, padding=True, return_tensors="pt").to(model.device)        # In case of video inputs, Add argument "videos=clip"

# Generate
start = time.time()
generate_ids = model.generate(**inputs, max_length=800)
print("Time taken : ", (time.time() - start))
output_conv = processor.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=True)[0]
print(output_conv.split("ASSISTANT:")[1].strip())
