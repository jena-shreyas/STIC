...
def preprocess_multimodal(
    ...
    for source in sources:
        for sentence in source:
            # print(sentence)
            # ======================================================================================================
            if sentence['content'].startswith(DEFAULT_IMAGE_TOKEN) or sentence['content'].startswith(DEFAULT_VIDEO_TOKEN):  # run with multi-im, multi-vid, multi-im & multi-vid
                ...
                IMAGE_TOKEN_NUM = sentence['content'].count(DEFAULT_IMAGE_TOKEN)
                if IMAGE_TOKEN_NUM > MAX_IMAGE_LENGTH:
                    sentence['content'] = sentence['content'].replace(DEFAULT_IMAGE_TOKEN * IMAGE_TOKEN_NUM, DEFAULT_IMAGE_TOKEN * MAX_IMAGE_LENGTH).strip()
                VIDEO_TOKEN_NUM = sentence['content'].count(DEFAULT_VIDEO_TOKEN)
                if VIDEO_TOKEN_NUM > MAX_VIDEO_LENGTH:
                    raise ValueError(f"{sentence['content']}")
            ...
            # print('before replace_token:', [sentence['content']])
            sentence["content"] = sentence["content"].replace(DEFAULT_IMAGE_TOKEN, replace_token)
            sentence['content'] = sentence['content'].replace(DEFAULT_VIDEO_TOKEN, vid_replace_token)
            # print('after replace_token:', [sentence['content']])
            # ======================================================================================================
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0] and 'video' not in sources[0]:
            ...
            sources = preprocess_multimodal(
                ...
                # rank0_print('video')
                ext = '.mp4'
                video_file = self.list_data_dict[i]['video'] + ext
                ...
                image = video_processor(video, return_tensors='pt')['pixel_values'][0]  # fake image
                # image = [torch.randn(3, 8, 224, 224) for i in video]  # fake image
                print(f"Index : {i}", end=" ")
                sources = preprocess_multimodal(copy.deepcopy([e["chosen"] for e in sources]), self.data_args)
...
def train():
    ...
    #     # Initialize image and video towers
    ...
    #         data_args.is_multimodal = True
        ...
        if model_args.video_tower is not None:
            print("Initializing video tower...")
        ...
        model.config.tokenizer_model_max_length = tokenizer.model_max_length if tokenizer_model_max_length is None else tokenizer_model_max_length   # 1048
    ...
    # Since, in DPO, we need to train the model wrt another reference model,
    # we need to load the reference model here.
    if model_args.image_tower is not None or model_args.video_tower is not None:
